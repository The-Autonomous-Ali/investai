...
{{
  ...
  "requires_immediate_analysis": true/false
}}
...
class SignalWatcherAgent:
    ...
    async def get_current_signals(self, limit: int = 10) -> dict:
        ...
        signals = (
            ...
            .filter(Signal.importance_score >= 5.0)
            .filter(Signal.detected_at >= datetime.utcnow() - timedelta(days=3))
        ...
        await self._cache_signals(result)
        return result
    ...
    async def _scan_rss(self, source: dict) -> list:
        ...
        for entry in feed.entries[:10]:  # limit to 10 most recent
            ...
            existing = self.db.query(Signal).filter(Signal.content_hash == content_hash).first()
            if existing:
                continue
    ...
    async def _classify_signal(self, content: str, source: str, tier: int) -> dict:
        """Use Claude to classify and extract signal data."""
        try:
    ...
    async def _get_market_snapshot(self) -> dict:
        """Get current market data snapshot. Uses cached data or fetches fresh."""
        cached = await self.redis.get("market_snapshot") if self.redis else None
        if cached:
            return json.loads(cached)

        # In production, this would call NSE API / Yahoo Finance
        # Returning mock structure here — replace with real API call
        snapshot = {
        ...
        if self.redis:
            await self.redis.setex("market_snapshot", 900, json.dumps(snapshot))  # 15 min TTL
    ...
    async def _cache_signals(self, data: dict):
        if self.redis:
            await self.redis.setex("top_signals", 900, json.dumps(data))  # 15 min TTL
