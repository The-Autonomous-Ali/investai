...
// Demo data used only when the live service is unavailable.
const MOCK_SIGNALS = [
  {
...
export function useSignals() {
    ...
    async function fetchSignals() {
      try {
        ...
        if (status === 401 || status === 403) {
          setDegradedReason('Sign in to see live signals. Showing demo data.')
        } else {
          setDegradedReason('Live signal service unavailable. Showing demo data.')
        }
      } finally {
        setLoading(false)
      }
  ...
  return { signals, loading, isLive, degradedReason }
}
